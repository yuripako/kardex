<?php $this->load->view($header); // aqui van los css y los modulos ?> 
       ...
       <canvas id="myAreaChart" height="0"  style="display: none;"></canvas> 
        ...
        <div class="card mb-3">
          <div class="card-header " style=" background-color: #00a65a;">
           ...
           <button data-toggle="modal"  data-toggle="modal" data-target="#modaladdunmedida" class="btn btn-warning"><i class="fas fa-plus "></i> Agregar Unidad</button>
          </div>
          <div class="card-body ">
            <div class="table-responsive">
              <table class="table table-hover" id="dataTable" width="100%" cellspacing="0">
                ...
                <tbody >
                <?php $cont=1; foreach ($unmedida as $item) { ?>  
                  <tr >
                     <td><?= $cont++; ?></td>
                     <td><?= $item->cod_unid; ?></td>
                     <td><?= $item->nom_unid; ?></td>
                     ...
                     <td class="text-center">
                        <div class="btn-group">
                            <button title="ELIMINAR unmedida" onclick="eliminar_unmedida('<?= $item->cod_unid ?>');" class="btn btn-danger" > <i class="fas fa-trash "></i></button>                          
                            <button title="EDITAR unmedida" onclick="actualizar_unmedida('<?= $item->cod_unid ?>','<?=$item->nom_unid ?>','<?=$item->cod_ubl ?>','<?=$item->descripcion ?>','<?=$item->estado ?>');"  data-toggle="modal" data-target="#modaleditarunmedida"  class="btn btn-warning"> <i class="fas fa-edit "></i></button>                                                  	
                        </div>
                    </td>
                  </tr>
                <?php } ?>
  ...
  <!-- Modal Agredar unmedida -->
<div class="modal fade" id="modaladdunmedida" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button onclick="agregar_unmedida();" type="button" class="btn btn-primary">Agregar Unidad</button>
      </div>
    </div>
  </div>
</div>
 ...
 <div class="modal fade" id="modaleditarunmedida" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Cerrar</button>
        <button onclick = "updateunmedida();" type="button" class="btn btn-primary">Guardar cambios</button>
      </div>
    </div>
  </div>
</div>
